...
import kotlinx.android.synthetic.main.fragment_formula_algorithm.*
...
class AlgorithmFragment : BaseFragment() {
    ...
    private val mActivity: FormulaActivity by lazy { requireActivity() as FormulaActivity }
    ...
    override fun getLayoutResID(): Int = R.layout.fragment_formula_algorithm
    ...
    override fun initListener() {
            ...
            val builder = CircleInputDialog.Builder(3)
                ...
                .setButtonOnClickListener { dialog, _, index ->
                    ...
                    if (countText.isEmpty()) {
                        ...
                        val count = countText.toInt()
                        when (index) {
                            1 -> mViewModel.removeLastItems(count, recyclerViewData, mDataAdapter)
                            2 -> mViewModel.removeToLastItems(count, recyclerViewData, mDataAdapter)
                        }
                    }
                    dialog.dismiss()
                }
        ...
        buttonCalculate.setOnClickListener {
            if (mViewModel.numbers.size < AlgorithmViewModel.MIN_ITEM_COUNT) {
                ...
                when (mActivity.aboutFragment.getMathFormula()) {
                    is MathFormulaRMD -> {
                        result.addAll(
