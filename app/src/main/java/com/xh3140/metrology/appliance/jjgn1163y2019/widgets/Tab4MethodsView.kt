...
class Tab4MethodsView : MethodsView {
    ...
    init {
        ...
        addView(createLayout(createTextView("2、心率示值误差", 15f)))
        ...
        addView(createLayout(createTextView("3、心率示值误差技术要求", 15f)))
        val request = "　　在(30～200)次/min范围内，最大允许误差为±(示值的5%+1)次/min。"
        addView(createLayout(createTextView(request, 13f)))
        addView(createLayout(createTextView("4、心率示值误差计算公式", 15f)))
        addView(createLayout(createSpace(), createImageView(dp2px(350), 3801, 1424, R.drawable.jjg_1163_2019_formula5), createSpace()))
    }
}